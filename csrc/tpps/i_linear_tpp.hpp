...
template <int reduction_count, i_format i_fmt> struct io_policy;
template <typename intermediate_type, typename io_policy> struct quant_policy;

template <typename io_policy>
struct quant_policy<float> : io_policy {
  inline static void quant_out(
      void *C, size_t ldc, void *s_0, void *s_1, float *bias,
    ...
    for (int t = 0; t < row_tile; ++ t) {
      ...
      for (int i = 0; i < 16; ++ i) {
        ...
        if (post_op) {
          ...
          io_policy::template out_policy<float>::
          _mm512_coalescing_packs_store(C_[t], ldc, i, o0, o1, o2, o3);
        } else {
...
template <typename io_policy>
struct quant_policy<_Float16> : io_policy {
  inline static void quant_out(
      void *C, size_t ldc, void *s_0, void *s_1, _Float16 *bias,
      float scale, bool post_op, float o_scale) {
    auto s_0_ = reinterpret_cast<int (*)[2][16][16]>(s_0);
    auto s_1_ = reinterpret_cast<int (*)[2][16][16]>(s_1);
    ...
    for (int t = 0; t < row_tile; ++ t) {
      ...
      for (int i = 0; i < 16; i += 2) {
        ...
        if (post_op) {
          ...
          // every 16 got output
          io_policy::template out_policy<_Float16>::
          _mm512_coalescing_packs_store(C_[t], ldc, i, o0, o1, o2, o3);
        } else {
...
template <int reduction_count>
struct io_policy<reduction_count, i_format::tile> {
  typedef int8_t (* tile_array)[16 * 64];
  typedef int8_t (* block_pointer)[reduction_count][16][64];
  ...
  template <typename interm_type> out_policy;

  template <> out_policy<float> {
    inline static void _mm512_coalescing_store(
        void* C, size_t ldc, int idx,
        __m512i o0, __m512i o1, __m512i o2, __m512i o3) {
...
template <int reduction_count>
struct io_policy<reduction_count, i_format::plain> {
  typedef int8_t (* tile_array)[64];
  typedef int8_t (* block_pointer)[16][reduction_count][64];

  template <int tile_num> 
  inline static void tile_load(void* A, int idx) {
    auto A_ = reinterpret_cast<block_pointer>(A);
    __tile_loadd<tile_num>(A_[idx], reduction_count * 64);
  }
  ...
  template <> out_policy<float> {
    ...
    inline static void _mm512_coalescing_packs_store(
        void* C, size_t ldc, int idx,
        __m512i o0, __m512i o1, __m512i o2, __m512i o3) {
      auto C_ = reinterpret_cast<int8_t (*)[ldc]>(C);
...
template <int row_tile, int reduction_count, typename io_policy>
struct _tile_dot_product_16x256;

template <int reduction_count, typename io_policy>
struct _tile_dot_product_16x256<1, reduction_count, io_policy> {
  static constexpr size_t row_tile = 1;
  static constexpr size_t A_footprint = row_tile * reduction_count * 16 * 64;
  static constexpr size_t B_footprint = reduction_count * 16 * 256;
  static constexpr size_t cache_footprint = A_footprint + B_footprint;
  static constexpr size_t lda = reduction_count * 64;

  inline static void dot_prod(void *A, void *B) {
    auto B_ = reinterpret_cast<int8_t (*)[reduction_count][16][64]>(B);
  ...
  // TODO: remove it when io_policy::out_policy is ready
  inline static void quant_out(
      void *C, size_t ldc, void *s_0, void *s_1, float *bias,
      float scale, bool post_op, float o_scale) {
    auto s_0_ = reinterpret_cast<int (*)[2][16][16]>(s_0);
...
template <int reduction_count, typename io_policy>
struct _tile_dot_product_16x256<2, reduction_count, io_policy> {
...
template <int reduction_count, typename io_policy>
struct _tile_dot_product_16x256 <3, reduction_count, io_policy> {
  ...
  inline static void quant_out(void *C, size_t ldc, void *s_0, void *s_1, float *bias,
      float scale) {
...
#   pragma unroll (4)
    for (int j = 0; j < 4; ++j) {
      zero_accum();
#     pragma unroll (reduction_count / 2)
      for (int i = 0; i < reduction_count / 2; i++) {
        dot_prod<TMM7>(A_[2 * i], B_[j][2 * i]);
        dot_prod<TMM6>(A_[2 * i + 1], B_[j][2 * i + 1]);
      }

      if (reduction_count % 2 == 1) {
        dot_prod<TMM7>(A_[reduction_count - 1], B_[j][reduction_count - 1]);
      }
...
template <int reduction_count, typename io_policy>
struct _tile_dot_product_16x256 <4, reduction_count, io_policy> {
...
template <int reduction_count, typename io_policy>
struct _tile_dot_product_16x256 <5, reduction_count, io_policy> {
...
template <int reduction_count, typename io_policy>
struct _tile_dot_product_16x256 <6, reduction_count, io_policy> {
...
public:
  i_linear(
      size_t sequence_length, size_t input_feature, size_t output_feature,
      bool bias, bool post_op
  ) : sl_(sequence_length), ic_(input_feature), oc_(output_feature),
    has_bias_(bias), post_op_(post_op) {
  ...
  void tile_linear(const int row_tile, size_t roll_back, const int reduction_count, 
                   void *C, void *A, void *B, float *bias, float scale, float o_scale);
...
public:
  ...
  template <int row_tile, int reduction_count>
  void compute_block(void* C, size_t ldc, void* A, void* B, _Float16* bias,
      float scale, bool post_op = false, float o_scale = 1.0);

  void tile_dot_product_16x256(void *C, void *A, void *B, _Float16 *bias,
      float scale, float o_scale, const size_t sl, const size_t col_step,
      size_t cur_id=0, size_t total_chunks=1);

  void tile_dot_product_16x256_shortage(void *C, void *A, void *B, _Float16 *bias,
      float scale, float o_scale, const size_t sl, const size_t col_step,
      size_t cur_id=0, size_t total_chunks=1);
