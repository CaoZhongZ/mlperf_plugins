...
std::tuple<at::Tensor, at::Tensor> i_layernorm_pad(const at::Tensor &input, const at::Tensor &weight,
...
at::Tensor i_layernorm(const at::Tensor &input, const at::Tensor &weight,
                       const at::Tensor &bias, const at::Scalar &oscale,
                       const c10::optional<at::Scalar> &eps,
  ...
  auto output =
      at::empty(in_sz, at::TensorOptions().dtype<int8_t>().memory_format(
...
#pragma omp parallel for
    for (auto i = 0; i < batch; ++i) {
      ...
      i_layernorm_tpp<16>::ref(pout[i], pin[i], pw, pb, oscale.toFloat(),
...
at::Tensor
i_residual_layernorm(const at::Tensor &input1, const at::Tensor &input2,
                     const at::Tensor &weight, const at::Tensor &bias,
                     const at::Scalar &scale_1, const at::Scalar &scale_2,
...
#pragma omp parallel for
  for (auto i = 0; i < batch; ++i) {
    ...
    i_residual_layernorm_tpp<16>::ref(pout[i], psrc1[i], psrc2[i], pw, pb,
                                      scale_1.toFloat(), scale_2.toFloat(),
                                      oscale.toFloat(), reduce_l,
...
#pragma omp parallel for
  for (auto i = 0; i < batch; ++i) {
    ...
    i_residual_layernorm_tpp<32>::ref_fp16(
        pout[i], psrc1[i], psrc2[i], pw, pb, scale_1.toFloat(),
        scale_2.toFloat(), oscale.toFloat(), reduce_l,
        eps.value_or(1e-12).toFloat(), _o_off);
  }
...
at::Tensor
i_residual_layernorm_cin_(at::Tensor &input1, const at::Tensor &input2,
                          const at::Tensor &weight, const at::Tensor &bias,
                          const at::Scalar &scale_1, const at::Scalar &scale_2,
                          const at::Scalar &oscale,
                          const c10::optional<at::Scalar> &eps,
