...
#pragma unroll NBlock
      for (int j = 0; j < NBlock; j++)
      {
        ...
        print_2d_matrix<int8_t>((int8_t *)&k[0][cur_k_pos], 16, 64, aks);
        getchar();
        _tile_loadd(TMM7, &k[0][cur_k_pos_r], aks);
        print_2d_matrix<int8_t>((int8_t *)&k[0][cur_k_pos_r], 16, 64, aks);
        getchar();
...
#pragma unroll NBlock
      for (int j = 0; j < NBlock; j++)
      {
        ...
        _tile_stored(TMM0, &a[0][cur_k_pos], aks);
        print_2d_matrix<int>(&a[0][cur_k_pos], 16, 16, aks / 4);
        getchar();
        _tile_stored(TMM1, &a[0][cur_k_pos_r], aks);
      }
...
#pragma unroll NBlock
      for (int i = 0; i < NBlock; i++)
      {
        ...
        print_2d_matrix<int8_t>((int8_t *)q[cur_q_pos], 16, 64, q_s);
        getchar();
        _tile_loadd(TMM5, q[cur_q_pos_b], q_s);
        print_2d_matrix<int8_t>((int8_t *)q[cur_q_pos_b], 16, 64, q_s);
        getchar();

        qk_gemm_impl<1, 32, NBlock>::run(k_ptr, a_buffer, ldq, sl_pad, rollback_);

        print_2d_matrix<int>((int *)a_buffer, 32, 8, sl_pad);
        getchar();
        // add softmax
