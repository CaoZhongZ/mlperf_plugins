...
// Since AMX tile is compute in 32-bit width, we use uniformly 32-bit
// as logic length units.
//
// Which means all the logic tile shape is 16 * 16 for the actual elements
// are 16 * 64
...
struct tilecfg {
  ...
  uint16_t tile_colsb[num_valid]; /* bytes 16-31 */
  char rsvd2[16];         /* bytes 32-47 */
  uint8_t tile_rows[num_valid];   /* bytes 48-55 */
...
inline status_t configure_tile(struct tilecfg *cfg, int ntile, int row,
                               int colsb) {
...
status_t reorder_k_to_buffer(const int8_t *k_ptr, int8_t *k_buffer, int row,
                             int row_pad, int col, int stride) {
  /*
...
template <int Rows, int Steps> struct av_gemm_post_impl {
  inline static void run(const void *apro, const void *v_buffer, void *r_buffer,
                         void *c_ptr, int32_t sl_pad, int32_t rollback,
                         float m2);
};

template <int Steps> struct av_gemm_post_impl<32, Steps> {
  inline static void run(const void *apro, const void *v_buffer, void *r_buffer,
                         void *c_ptr, int32_t sl_pad, int32_t rollback,
                         float m2) {
...
template <int Steps> struct av_gemm_post_impl<16, Steps> {
  inline static void run(const void *apro, const void *v_buffer, void *r_buffer,
                         void *c_ptr, int32_t sl_pad, int32_t rollback,
                         float m2) {
    auto a = reinterpret_cast<const int8_t(*)[sl_pad]>(apro);
...
template <int rows>
inline void av_gemm_kernel(const void *apro, const void *v_buffer,
                           void *r_buffer, void *c_ptr, int32_t sl_pad,
                           int32_t rollback, int32_t step, float m2);

template <>
inline void av_gemm_kernel<32>(const void *apro, const void *v_buffer,
                               void *r_buffer, void *c_ptr, int32_t sl_pad,
                               int32_t rollback, int32_t step, float m2) {
  // do 32 row gemm
  // a shape [32, sl_pad]
  ...
  switch (step) {
  case (1):
    av_gemm_post_impl<32, 1>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (2):
    av_gemm_post_impl<32, 2>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (3):
    av_gemm_post_impl<32, 3>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (4):
    av_gemm_post_impl<32, 4>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (5):
    av_gemm_post_impl<32, 5>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (6):
    av_gemm_post_impl<32, 6>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (8):
    av_gemm_post_impl<32, 8>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (17):
    av_gemm_post_impl<32, 17>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                   rollback, m2);
    break;
  case (19):
    av_gemm_post_impl<32, 19>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                   rollback, m2);
    break;
  case (23):
    av_gemm_post_impl<32, 23>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                   rollback, m2);
    break;
  }
}
...
template <>
inline void av_gemm_kernel<16>(const void *apro, const void *v_buffer,
                               void *r_buffer, void *c_ptr, int32_t sl_pad,
                               int32_t rollback, int32_t step, float m2) {
  // do 32 row gemm
  // a shape [32, sl_pad]
  ...
  switch (step) {
  case (1):
    av_gemm_post_impl<16, 1>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (2):
    av_gemm_post_impl<16, 2>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (3):
    av_gemm_post_impl<16, 3>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (4):
    av_gemm_post_impl<16, 4>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (5):
    av_gemm_post_impl<16, 5>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (6):
    av_gemm_post_impl<16, 6>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (8):
    av_gemm_post_impl<16, 8>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                  rollback, m2);
    break;
  case (17):
    av_gemm_post_impl<16, 17>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                   rollback, m2);
    break;
  case (19):
    av_gemm_post_impl<16, 19>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                   rollback, m2);
    break;
  case (23):
    av_gemm_post_impl<16, 23>::run(apro, v_buffer, r_buffer, c_ptr, sl_pad,
                                   rollback, m2);
    break;
  }
}

// row_tile: 1 or 2, col_tile: 3, ..., 24 (384, could be more)
template <int row_tile, int col_tile> struct qk_gemm_impl {
  ...
  template <bool tail> inline static tile_loadb(const void *b, int col_idx) {
    auto b_ = reinterpret_cast<const int(*)[16]>(b);
    _tile_loadd(TMM6, b_[col_idx], ldb * 4);
    if (!tail)
      _tile_loadd(TMM7, b_[col_idx + 1], ldb * 4);
  }
  ...
  template <bool tail>
  inline static dot_prod(void *c, int col_idx, int overlap) {
  ...
  // Preloaded A
  inline static void compute(void *c, const void *b, const void *a,
                             int overlap) {
...
#   pragma unroll(col_loop)
  ...
  inline static void softmax(void *c_int8, void *c, int len, float M,
                             float oscale, int overlap) {
    ...
    i32_scale_attlen_softmax_scale_i8<16, 16>(c_int8_[0], c_[0], len, M, oscale,
                                              ldb);
...
template <int n_tile, int k_tile> struct av_gemm_impl {
  constexpr size_t ldb = 64;
  constexpr size_t lscratch = 256;
  constexpr size_t ldc = 64;

  inline void config(struct tilecfg *cfg, int k) {
    // input A tiles
    configure_tile(cfg, 4, 16, k * 4);
    configure_tile(cfg, 5, 16, k * 4);
    // input B tiles
    configure_tile(cfg, 6, k, 64);
    configure_tile(cfg, 7, k, 64);
    // output tiles
    configure_tile(cfg, 0, 16, 64);
    configure_tile(cfg, 1, 16, 64);
    configure_tile(cfg, 2, 16, 64);
  ...
  inline void loada(void *a_scratch, size_t idx, size_t k_block, size_t overlap) {
    // a shape int [n_tile][16][k_tile][k_block], k_tile * b_block = lda

    auto a_ = reinterpret_cast<int (*)[16-overlap][k_tile][k_block]>(a_scratch);

    _tile_loadd(TMM4, a_[0][idx], k_tile * b_block);
    if (n_tile == 2) _tile_loadd(TMM5, a[1][idx], k_tile * b_block);
  }

  inline void loadb(void *b_scratch, size_t idx) {
    // b shape int [2][k_tile][k_block][32]
    auto b_ = reinterpret_cast<int(*)[k_tile][k_block][32]>(b_scratch);

    _tile_loadd(TMM6, b_[0][idx], lscratch);
    _tile_loadd(TMM7, b_[1][idx], lscratch);
  }
  ...
  inline void store_quant(void *c, size_t overlap, float m2, int off) {
    alignas(64) int scratch[n_tile * 16][2][16];

    _tile_stored(TMM0, scratch[0], 32 * 4);
    _tile_stored(TMM1, scratch[0] + 1, 32 * 4);

    if (n_tile == 2) {
      _tile_stored(TMM2, scratch[16 - overlap], 32 * 4);
      _tile_stored(TMM3, scratch[16 - overlap] + 1, 32 * 4);
    }

    // quant out to c
    auto vscale = _mm512_set1_ps(m2);
    auto c_out = reinterpret_cast<int8_t (*)[4][16]>(c);

    auto q_rows = n_tile * 16 - overlap;

    for (int i = 0; i < q_rows; i++) {
      auto pr = _mm512_load_epi32(scratch[i][0]);
      auto pr1 = _mm512_load_epi32(sratch[i][1]);

      auto prf = _mm512_cvtepi32_ps(pr);
      auto prf1 = _mm512_cvtepi32_ps(pr1);

      auto iout = _mm512_scale_minmax_i8_ps(vscale, prf);
      auto iout1 = _mm512_scale_minmax_i8_ps(vscale, prf1);

      // 0, 1 or 2, 3
      _mm512_mask_cvtepi32_storeu_epi8(c_out[i][off], 0xffff, iout);
      _mm512_mask_cvtepi32_storeu_epi8(c_out[i][off + 1], 0xffff, iout1);
    }
  }

  inline void compute(
      void *c_scratch, void *a_scratch, size_t lda,
      void *b_scratch, size_t overlap, float m2) {
    auto k_block = lda / k_tile;
    ...
    // c shape int [2][n_tile][16][32]
    auto c_ = reinterpret_cast<int (*)[n_tile][16][32]>(c_scratch);

    zero_accum();
#   pragma unroll(k_step)
    for (int i = 0; i < k_tile; ++i) {
      loada(a, i, k_block, overlap);
      loadb(b_, i);
      dot_prod();
    }
    // First half
    store_quant(c_[0], m2, 0);
...
at::Tensor amx_mha(const at::Tensor &qkv, const at::Tensor &att_mask,
  ...
  for (int i = 0; i < bs; i++) // batch size
  {
    for (int j = 0; j < head_num; j++) // head num
    {
      ...
      reorder_k_to_buffer_v2(cur_k_ptr, cur_v_ptr, k_buffer, v_buffer, sl,
                             sl_pad, head_size, stride);
      auto time_point_2 = Time::now();
      copy_time += std::chrono::duration_cast<std::chrono::nanoseconds>(
                       time_point_2 - time_point_1)
                       .count();

      amx_per_head(cur_q_ptr, k_buffer, att_buffer, v_buffer, att_pro_buffer,
                   r_buffer, cur_a_ptr, nbq_row, stride, max_tile_row,
                   max_tile_row, rollback, m1.toFloat(), oscale.toFloat(),
                   att_mask_p[i], m2.toFloat());
      auto time_point_3 = Time::now();

      amx_time += std::chrono::duration_cast<std::chrono::nanoseconds>(
                      time_point_3 - time_point_2)
                      .count();
    }
  }
  auto loop_during = std::chrono::duration_cast<std::chrono::milliseconds>(
                         Time::now() - loop_start)
                         .count();

  std::cout << "init during : " << (float)init_during / 1000 / 1000 << " ms"
            << std::endl;
  std::cout << "-----------copy time: " << (float)copy_time / 1000 / 1000
            << " ms--------------" << std::endl;
  std::cout << "-----------amx time: " << (float)amx_time / 1000 / 1000
            << " ms--------------" << std::endl;
  std::cout << "-----------loop time: " << loop_during << " ms--------------"
            << std::endl;
  std::cout << "-----------other time: " << (float)other_during / 1000 / 1000
            << " ms--------------" << std::endl;
