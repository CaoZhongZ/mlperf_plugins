...
struct amx_qk_gemm_impl {
    inline static void run (void* q_ptr, void* k_ptr, void* a_ptr, int lda, int rollback);
};

template <int NBlock>
struct amx_qk_gemm_impl<1, NBlock> {
    inline static void run (void* q_ptr, void* k_ptr, void* a_ptr, int ldq, int rollback) {
        // sl / 16 is even
...
#       pragma unroll NBlock
        for (int i = 0; i < NBlock; i++) {
            ...
            int rollback_ = (i == mhad.nBlock - 1) ? rollback : 0;
            int cur_q_pos_b = cur_q_pos + mhad.q_block - rollback;

            _tile_loadd(TMM4, &q[cur_q_pos][0], q_s);
            _tile_loadd(TMM5, &q[cur_q_pos_b][0], q_s);
#           pragma unroll NBlock
            for (int j = 0; j < NBlock; j++) {
                int cur_k_pos = j * mhad.k_block * 2;
                int cur_k_pos_r = cur_k_pos + mhad.k_block;
                // coordinate of A
                int cur_a_pos[2] = {i * mhad.a_r_block * 2, j * mhad.a_c_block * 2};
                int cur_a_pos_r[2] = {i * mhad.a_r_block * 2, j * mhad.a_c_block * 2 + mhad.a_c_block};
                int cur_a_pos_b[2] = {i * mhad.a_r_block * 2 + mhad.a_r_block, j * mhad.a_c_block * 2};
                int cur_a_pos_rb[2] = {i * mhad.a_r_block * 2 + mhad.a_r_block, j * mhad.a_c_block * 2 + mhad.a_c_block};
...
struct amx_qk_gemm_impl<0, NBlock> {
    inline static void run (void* q_ptr, void* k_ptr, void* a_ptr, MHA_desc& mhad) {
        // sl / 16 is odd
        ...
        int q_s = mhad.qkv_stride_;
        int k_s = mhad.sl_pad * 4;
        int a_s = mhad.att_stride_ * mhad.typesize_A;
        ...
        int rollback = max_tile_row - mhad.q_tail;

#       pragma unroll NBlock
        for (int i = 0; i < NBlock; i++) {
            cur_q_pos = i * mhad.q_block * 2;
            cur_q_pos_b = cur_q_pos + mhad.q_block;
...
#           pragma unroll NBlock
            for (int j = 0; j < NBlock; j++) {
                cur_k_pos = j * mhad.k_block * 2;
                cur_k_pos_r = cur_k_pos + mhad.k_block;
                // coordinate of A
                cur_a_pos[2] = {i * mhad.a_r_block * 2, j * mhad.a_c_block * 2};
                cur_a_pos_r[2] = {i * mhad.a_r_block * 2, j * mhad.a_c_block * 2 + mhad.a_c_block};
                cur_a_pos_b[2] = {i * mhad.a_r_block * 2 + mhad.a_r_block, j * mhad.a_c_block * 2};
                cur_a_pos_rb[2] = {i * mhad.a_r_block * 2 + mhad.a_r_block, j * mhad.a_c_block * 2 + mhad.a_c_block};
            ...
            cur_k_pos += mhad.k_block * 2;
            cur_a_pos[1] += mhad.a_c_block * 2;
            cur_a_pos_b[1] += mhad.a_c_block * 2;
        ...
        // the last 16 tile
        cur_q_pos += mhad.q_block - rollback;
        cur_a_pos[0] += mhad.a_r_block - rollback;
        cur_a_pos_r[0] += mhad.a_r_block - rollback;
        _tile_loadd(TMM4, &q[cur_q_pos][0], q_s);
...
#       pragma unroll NBlock
        for (int j = 0; j < NBlock; j++) {
            ...
            cur_a_pos[1] = j * mhad.a_c_block * 2;
            cur_a_pos_r[1] = j * mhad.a_c_block * 2 + mhad.a_c_block;
